...
packages <- c("ks", "lattice", "adehabitatHR", "maptools", "foreign", "plot3D","rasterVis", "colorRamps","sf","terra")
...
crs(elc_habitat,proj = TRUE)
crs(wolfyht, proj = TRUE)
```
...
```{r, obj3}
elc_habitat <- project(elc_habitat, "+proj=longlat +datum=NAD83 +no_defs +ellps=GRS80 +towgs84=0,0,0")
...
writeVector(elc_habitat, here::here("Output","elc_habitat_NEW"), filetype = "ESRI Shapefile", insert = FALSE, overwrite = TRUE)
```

## Extents
Spatial data objects have a spatial extent, in geographic space, that they occupy.  To look at the spatial extent of the SpatVectore,we use ?ext()
```{r, extent}
ext(elc_habitat)
ext(wolfyht)
```
...
# look at raster data -- not sure how to do this one
data(deer_w)
